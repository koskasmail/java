...
* variable is getting assigned a value using `=` operator
...
* Variable names can contain `letters`, `digits`, `underscores`, `dollar signs` (`$`).
* Java is `case-sensitive` so variable named `myVar` and `MyVar` are different variable.
...
#### A variable `myWorkPc` is getting assigned a value using `=` operator
```java
int age;
age = 33;
```

##### variable is getting assigned a value while he is being declared
```java
int myAge;
myAge = 33;
myAge = 45;
...
| Primitive | #1 | #2 | #3 | #4 | description | example |
| :-: | :-: | :-: | :-: | :-: | --- | --- | 
| boolean | boolean | --- | --- | --- | --- | 0,1,true,false |  
| character | char | --- | --- | --- | --- | 'a' |  
| integers | byte | short | int | long | whole numbers | 333 | 
| float | float | double | --- | --- | real numbers with a decimal points | 1.23 | 
...
### 📊 Primitive Data Type Details (Name, Type, Range, Size)

| Name     | Type     | Range / Value             | Size         | Description |
|----------|----------|---------------------------|--------------|-------------|
| `boolean`| boolean  | `true` / `false`          | 1 bit        | Logical values |
| `byte`   | number   | -128 to 127               | 8 bit / 1 byte/s  | Small integers |
| `short`  | number   | -32,768 to 32,767         | 16 bit / 2 byte/s | Medium integers |
| `int`    | number   | -2,147,483,648 to 2,147,483,647 | 32 bit / 4 byte/s | Standard integers |
| `long`   | number   | Very large range          | 64 bit / 8 byte/s | Large integers |
| `float`  | decimal  | ~6 decimal digits         | 32 bit / 4 byte/s | Real numbers |
| `double` | decimal  | ~15 decimal digits        | 64 bit / 8 byte/s | High-precision real numbers |
| `char`   | character| Unicode characters        | 16 bit / 2 byte/s | Single letters/symbols |
